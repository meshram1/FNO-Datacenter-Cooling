"""Train the multi-field FNO2d on one-step Boussinesq prediction.

    python train_bsq.py --data ../data/boussinesq_64.npz --epochs 50

Reports per-field relative L2 (w and T separately) so you can see each field learn.
"""
import argparse
import json
import time
...
from dataset_bsq import load_boussinesq


def field_rel_l2(pred, true):
    """Per-field relative L2 -> (C,). pred/true: (B, H, W, C). Differentiable."""
    B, _, _, C = pred.shape
...
def evaluate(model, loader, device):
    ...
    for x, y in loader:
        ...
        n += w
    return (tot / n).tolist()          # [err_w, err_T]
...
def main():
    ...
    ap.add_argument("--out", default="bsq_fno.pt")
    ap.add_argument("--metrics", default="bsq_metrics.json")
    args = ap.parse_args()
    ...
    train_ds, test_ds, norm = load_boussinesq(args.data, args.t_in)
    C = train_ds.C
    train_loader = DataLoader(train_ds, batch_size=args.batch, shuffle=True)
    test_loader = DataLoader(test_ds, batch_size=args.batch, shuffle=False)

    model = FNO2d(args.t_in, C, args.modes, args.modes, args.width, args.layers).to(device)
    ...
    for ep in range(args.epochs):
        ...
        for x, y in train_loader:
            ...
            fl = field_rel_l2(model(x), y)         # (C,)
            loss = fl.mean()                       # equal weight to each field
            loss.backward()
            opt.step()
            w = x.shape[0]
            ...
            n += w
        sched.step()
        tr = (run / n).tolist()                    # [train_w, train_T]
        te_err = evaluate(model, test_loader, device)   # [test_w, test_T]
    ...
    torch.save({"model_state": model.state_dict(),
                "config": {"t_in": args.t_in, "n_fields": C, "modes": args.modes,
